// app/dashboard/_components/BuildingsPage.tsx
...
// ─── Data helpers (preserved from original, optimized) ────────────────────────
...
// ─── Empty state ──────────────────────────────────────────────────────────────
function EmptyBuildingState({ country, message }: { country: CountryConfig; message: string }) {
  const exportCmd = "python tools\\export_uae_module5_frontend_data.py"; // UAE only in current scope
  return (
    <div className="flex flex-col items-center justify-center px-4 py-24 text-center">
      ...
      <h1 className="text-xl font-bold" style={{ color: T.t1 }}>
        {country.label} Building Intelligence not available
      </h1>
...
// ─── Building Visibility Pulse featured card ──────────────────────────────────
function BuildingVisibilityPulse({
  totalRows,
  withPressure,
  withDominance,
}: {
  ...
  const insights = [
    `${formatNumber(totalRows)} building records in this export`,
    ...
    >
      ...
      <div className="relative p-6 sm:p-7">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-[20px] font-bold tracking-tight" style={{ color: T.t1 }}>
              Building Visibility Pulse
            </h2>
            <p className="mt-1 text-[13px]" style={{ color: T.t3 }}>
              Public listing coverage and concentration at building level.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          ...
          {/* Abstract building skyline visual */}
...
// ─── Building Lane Card (absolute left bar) ───────────────────────────────────
function BuildingLaneCard({
  icon: Icon,
  title,
  description,
  statLabel,
...
// ─── Building Watchlist Row ───────────────────────────────────────────────────
function BuildingWatchCard({ card }: { card: BuildingCard }) {
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-2 mt-0.5">
          <span className="inline-flex items-center gap-1 text-[11px]" style={{ color: T.t4 }}>
            ...
            {joinParts([card.city, card.community])}
          </span>
  ...
  return (
    <div className="rounded-2xl border p-6" style={{ background: T.cardBg, borderColor: T.border, boxShadow: "0 2px 10px rgba(0,0,0,0.22)" }}>
      <div className="flex items-start gap-4">
        ...
        <div>
          <h3 className="text-base font-semibold" style={{ color: T.t1 }}>
            Building intelligence coverage is limited for KSA
          </h3>
          <p className="mt-2 text-[13px] leading-relaxed" style={{ color: T.t3 }}>
            KSA building-level data is not yet available in the current Module 5 export schema.
            Building intelligence for KSA will be expanded as public listing coverage and
            address normalisation improve. In the meantime, you can explore city and district
            intelligence or review market-wide activity.
          </p>
...
export default function BuildingIntelligencePage({
  ...
  // For KSA, building intelligence may be absent; we show limited coverage.
  ...
  if (country.slug === "ksa" && isLimitedKSA) {
    // KSA limited coverage — still show header and footer
    return (
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-[14px] leading-relaxed" style={{ color: T.t2 }}>
              Building‑level intelligence is staged for KSA. Current data covers cities and districts.
            </p>
          </div>
  ...
  const cards: BuildingCard[] = payload.items.map((record, index) =>
    normalizeBuildingCard(country, record, index)
  );
  ...
  const metricTotalBuildings = formatNumber(totalRows);
  ...
  const locationTerm = country.slug === "uae" ? "buildings" : "cities/districts";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-[14px] leading-relaxed" style={{ color: T.t2 }}>
            Review building‑level public listing visibility, inventory concentration, and coverage signals.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          value={metricTotalBuildings}
          description={`Building records available in ${locationTerm}.`}
          tone="neutral"
          visual="bars"
        />
        <MetricCard
          ...
          description="Total active listings across visible building sample."
          tone="emerald"
          visual="bars"
        />
        <MetricCard
      ...
      {/* ── Building Visibility Pulse featured section ──────────────────── */}
      <BuildingVisibilityPulse totalRows={totalRows} withPressure={withPressure} withDominance={withDominance} />

      {/* ── Building intelligence lanes ─────────────────────────────────── */}
      <div className="grid gap-5 md:grid-cols-3">
        <BuildingLaneCard
          icon={Building2}
          title="Building Visibility"
          description="Public listing volume, agency presence, and concentration signals at building level."
          statLabel="Buildings"
          statValue={metricTotalBuildings}
          ctaHref={`${country.routeBase}/buildings`}
          ctaText="View buildings"
          tone="neutral"
        />
        <BuildingLaneCard
          icon={TrendingDown}
        ...
        <BuildingLaneCard
          ...
          description="Top agency footprint and listing-share concentration by building."
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="mt-1 text-[13px]" style={{ color: T.t3 }}>
                Top {visibleCards.length} buildings with visible listing activity.
              </p>
            </div>
            <span
            ...
            >
              {metricTotalBuildings} total
            </span>
          </div>
          <div className="space-y-2">
            {visibleCards.map((card) => (
              <BuildingWatchCard key={card.id} card={card} />
            ))}
          </div>
